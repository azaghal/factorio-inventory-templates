...
function handlers.on_configuration_changed(data)
    local inventory_templates_changes = data.mod_changes["inventory-templates"]

    if inventory_templates_changes then
        main.update_data(inventory_templates_changes.old_version,
                         inventory_templates_changes.new_version)
    end
end
